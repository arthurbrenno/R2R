...
class TestBuildStandardColumnCondition:
    ...
    @pytest.mark.parametrize(
        "op, value, expected_sql, expected_params",
        [
    ...
    def test_unsupported_operator(self):
        ...
        with pytest.raises(FilterError, match="Unsupported operator"):
            _build_standard_column_condition(
                "col", FilterOperator.OVERLAP, [], helper
            )
    ...
    def test_invalid_value_type_for_like(self):
        ...
        with pytest.raises(FilterError, match="requires a string value"):
            _build_standard_column_condition(
                "col", FilterOperator.LIKE, 123, helper
            )
        with pytest.raises(FilterError, match="requires a string value"):
            _build_standard_column_condition(
                "col", FilterOperator.ILIKE, 123, helper
            )
    ...
    def test_invalid_value_type_for_list_ops(self):
        ...
        with pytest.raises(FilterError, match="requires a list value"):
            _build_standard_column_condition(
                "col", FilterOperator.IN, "not-a-list", helper
            )
        with pytest.raises(FilterError, match="requires a list value"):
            _build_standard_column_condition(
                "col", FilterOperator.NIN, "not-a-list", helper
            )
...
class TestBuildCollectionIdsCondition:
    ...
    @pytest.mark.parametrize(
        ...
        [
            ...
            (
                FilterOperator.NIN,
                [UUID1],
                "NOT (collection_ids && ARRAY[$1]::uuid[])",
                [UUID1],
            ),
            (
                FilterOperator.NIN,
                [UUID1, UUID2],
                "NOT (collection_ids && ARRAY[$1,$2]::uuid[])",
                [UUID1, UUID2],
            ),
            (FilterOperator.NIN, [], "TRUE", []),
            (
    ...
    def test_operators(self, op, value, expected_sql, expected_params):
        helper = ParamHelper()
        sql_direct = _build_collection_ids_condition(
            "collection_ids", op, value, helper
        )
    ...
    def test_invalid_uuid(self):
        ...
        with pytest.raises(FilterError, match="Invalid UUID format"):
            _build_collection_ids_condition(
                "collection_ids", FilterOperator.OVERLAP, ["invalid"], helper
            )
        with pytest.raises(FilterError, match="Invalid UUID format"):
            _build_collection_ids_condition(
                "collection_ids",
        ...
        with pytest.raises(FilterError, match="Invalid UUID format"):
            _build_collection_ids_condition(
                "collection_ids", FilterOperator.EQ, "invalid", helper
            )
    ...
    def test_invalid_value_type_list(self):
        ...
        with pytest.raises(FilterError, match="requires a list"):
            _build_collection_ids_condition(
                "collection_ids", FilterOperator.OVERLAP, UUID1, helper
            )
        with pytest.raises(FilterError, match="requires a list"):
            _build_collection_ids_condition(
                "collection_ids", FilterOperator.ARRAY_CONTAINS, UUID1, helper
            )
    ...
    def test_invalid_value_type_single(self):
        ...
        with pytest.raises(FilterError, match="requires a single UUID"):
            _build_collection_ids_condition(
                "collection_ids", FilterOperator.EQ, [UUID1], helper
            )
        with pytest.raises(FilterError, match="requires a single UUID"):
            _build_collection_ids_condition(
                "collection_ids", FilterOperator.NE, [UUID1], helper
            )
    ...
    def test_unsupported_operator(self):
        ...
        with pytest.raises(FilterError, match="Unsupported operator"):
            _build_collection_ids_condition(
                "collection_ids", FilterOperator.GT, [UUID1], helper
            )
...
class TestBuildMetadataCondition:
    ...
    def _expected_safe_compare_sql(
        self, accessor, sql_op, param_placeholder, cast_type="numeric"
    ):
    ...
    @pytest.mark.parametrize(
        ...
        [
            ...
            (
                FilterOperator.NIN,
                ["a", "b"],
                f"NOT ({JSON_COLUMN}->'key' ?| ARRAY[$1,$2]::text[])",
                ["a", "b"],
            ),  # JSONB array op
            (FilterOperator.NIN, [], "TRUE", []),
            (
    ...
    def test_operators_simple_path(
        ...
        sql = _build_metadata_condition(
            "key", condition_spec, helper, self.json_col
        )
        ...
        if isinstance(value, bool) and op in [
            ...
            expected_sql_full = self._expected_safe_compare_sql(
                accessor, sql_op_map[op], "$1", "boolean"
            )
        elif (
            ...
            expected_sql_full = self._expected_safe_compare_sql(
                accessor, sql_op_map[op], "$1", "numeric"
            )
        elif value == [] and op == FilterOperator.IN:
            expected_sql_full = "FALSE"
        elif value == [] and op == FilterOperator.NIN:
    ...
    @pytest.mark.parametrize(
        ...
        [
            ...
            (
                "details.metrics.score",
                FilterOperator.GTE,
                95.5,
                None,
                [95.5],
            ),  # Deeper Numeric Safe Compare
            (
                "details.flags",
                FilterOperator.JSON_CONTAINS,
                ["urgent", "review"],
                f'#>\'{{"details","flags"}}\' @> $1::jsonb',
                [json.dumps(["urgent", "review"])],
            ),
        ],
    )
    def test_operators_nested_path(
        self, path, op, value, expected_sql_part, expected_params
    ):
        helper = ParamHelper()
        condition_spec = {op: value}
        # This function should add the CORRECTLY encoded param to helper.params
        sql = _build_metadata_condition(
            path, condition_spec, helper, self.json_col
        )
        ...
        if isinstance(value, bool) and op in [
            ...
            sql_op_map = {FilterOperator.EQ: "=", FilterOperator.NE: "!="}
            expected_sql_full = self._expected_safe_compare_sql(
                text_accessor, sql_op_map[op], "$1", "boolean"
            )
        elif (
            isinstance(value, (int, float))
            and not isinstance(value, bool)
            ...
            expected_sql_full = self._expected_safe_compare_sql(
                text_accessor, sql_op_map[op], "$1", "numeric"
            )
        elif value == [] and op == FilterOperator.IN:
            expected_sql_full = "FALSE"
        elif value == [] and op == FilterOperator.NIN:
            ...
            expected_sql_full = (
                f"{jsonb_accessor_prefix}{jsonb_accessor_suffix} @> $1::jsonb"
            )
    ...
    def test_eq_shorthand_nested_path(self):
        ...
        condition_spec = "value"
        sql = _build_metadata_condition(
            "p1.p2", condition_spec, helper, self.json_col
        )
    ...
    def test_nested_structure_condition(self):
        helper = ParamHelper()
        condition_spec = {"p2": "value"}
        sql = _build_metadata_condition(
            "p1", condition_spec, helper, self.json_col
        )
    ...
    def test_nested_structure_condition_with_op(self):
        helper = ParamHelper()
        condition_spec = {"p2": {FilterOperator.GT: 5}}
        sql = _build_metadata_condition(
            "p1", condition_spec, helper, self.json_col
        )
        # Correctly resolves to filtering on p1.p2 with GT
        accessor = f'{self.json_col}#>>\'{{"p1","p2"}}\''
        expected_sql = self._expected_safe_compare_sql(
            accessor, ">", "$1", "numeric"
        )
        assert sql.replace(" ", "") == expected_sql.replace(" ", "")
    ...
    def test_null_handling_simple(self):
        ...
        sql_eq = _build_metadata_condition(
            "key", {FilterOperator.EQ: None}, helper_eq, self.json_col
        )
        expected_sql_eq = f"{self.json_col}->>'key' IS NULL"
        assert sql_eq.replace(" ", "") == expected_sql_eq.replace(" ", "")
        assert helper_eq.params == []
        helper_ne = ParamHelper()
        sql_ne = _build_metadata_condition(
            "key", {FilterOperator.NE: None}, helper_ne, self.json_col
        )
        expected_sql_ne = f"{self.json_col}->>'key' IS NOT NULL"
    ...
    def test_null_handling_nested(self):
        helper_eq = ParamHelper()
        sql_eq = _build_metadata_condition(
            "p1.p2", {FilterOperator.EQ: None}, helper_eq, self.json_col
        )
        expected_sql_eq = f'{self.json_col}#>>\'{{"p1","p2"}}\' IS NULL'
        assert sql_eq.replace(" ", "") == expected_sql_eq.replace(" ", "")
        assert helper_eq.params == []
        helper_ne = ParamHelper()
        sql_ne = _build_metadata_condition(
            "p1.p2", {FilterOperator.NE: None}, helper_ne, self.json_col
        )
        expected_sql_ne = f'{self.json_col}#>>\'{{"p1","p2"}}\' IS NOT NULL'
        assert sql_ne.replace(" ", "") == expected_sql_ne.replace(" ", "")
        assert helper_ne.params == []
    ...
    @pytest.mark.parametrize(
        ...
        [
            ...
            (
                FilterOperator.NIN,
                ["a", "b"],
                f"NOT ({JSON_COLUMN}->'tags' ?| ARRAY[$1,$2]::text[])",
                ["a", "b"],
            ),
            (
                FilterOperator.NIN,
                ["single"],
                f"NOT ({JSON_COLUMN}->'tags' ?| ARRAY[$1]::text[])",
                ["single"],
            ),
            (FilterOperator.NIN, [], "TRUE", []),
        ],
    )
    def test_jsonb_array_operators_simple_path(
        self, op, value, expected_sql_part, expected_params
    ):
        helper = ParamHelper()
        condition_spec = {op: value}
        sql = _build_metadata_condition(
            "tags", condition_spec, helper, self.json_col
        )
        expected_sql_full = ""
        if op == FilterOperator.IN and not value:
            expected_sql_full = "FALSE"
        elif op == FilterOperator.NIN and not value:
            expected_sql_full = "TRUE"
        elif op == FilterOperator.NIN:
            expected_sql_full = (
                expected_sql_part  # NOT is part of expected_sql_part
            )
        else:
            expected_sql_full = (
                f"{self.json_col}{expected_sql_part}"  # Uses -> accessor
            )
        assert sql.replace(" ", "") == expected_sql_full.replace(" ", "")
        assert helper.params == expected_params
    ...
    @pytest.mark.parametrize(
        ...
        [
            (
                FilterOperator.IN,
                ["legacy"],
                f'#>\'{{"version","tags"}}\' ?| ARRAY[$1]::text[]',
                ["legacy"],
            ),
            (
                FilterOperator.IN,
                ["stable", "beta"],
                f'#>\'{{"version","tags"}}\' ?| ARRAY[$1,$2]::text[]',
                ["stable", "beta"],
            ),
            (FilterOperator.IN, [], "FALSE", []),
            (
                FilterOperator.NIN,
                ["legacy"],
                f'NOT ({JSON_COLUMN}#>\'{{"version","tags"}}\' ?| ARRAY[$1]::text[])',
                ["legacy"],
            ),
            (
                FilterOperator.NIN,
                ["stable", "beta"],
                f'NOT ({JSON_COLUMN}#>\'{{"version","tags"}}\' ?| ARRAY[$1,$2]::text[])',
                ["stable", "beta"],
            ),
            (FilterOperator.NIN, [], "TRUE", []),
        ],
    )
    def test_jsonb_array_operators_nested_path(
        self, op, value, expected_sql_part, expected_params
    ):
        helper = ParamHelper()
        condition_spec = {op: value}
        sql = _build_metadata_condition(
            "version.tags", condition_spec, helper, self.json_col
        )
        expected_sql_full = ""
        if op == FilterOperator.IN and not value:
            expected_sql_full = "FALSE"
    ...
    def test_unsupported_operator(self):
        ...
        condition_spec = {
            FilterOperator.OVERLAP: []
        }  # OVERLAP not supported for general metadata
        with pytest.raises(FilterError, match="Unsupported operator"):
            _build_metadata_condition(
                "key", condition_spec, helper, self.json_col
            )
    ...
    def test_json_contains_non_serializable(self):
        helper = ParamHelper()
        condition_spec = {
            FilterOperator.JSON_CONTAINS: {"a": {1, 2}}
    ...
    def test_contains_operator_maps_to_json_contains_nested(self):
        ...
        sql = _build_metadata_condition(
            "details.flags", condition_spec, helper, self.json_col
        )
        expected_sql = (
            f'{self.json_col}#>\'{{"details","flags"}}\' @> $1::jsonb'
        )
        assert sql.replace(" ", "") == expected_sql.replace(" ", "")
...
class TestProcessFieldCondition:
    ...
    def test_routes_collection_id_shorthand_single_value(self):
        helper = ParamHelper()
        sql = _process_field_condition(
            "collection_id", UUID1, helper, self.top_cols, self.json_col
        )
    ...
    def test_routes_collection_ids_direct_op(self):
        ...
        sql = _process_field_condition(
            "collection_ids",
            {FilterOperator.OVERLAP: [UUID1, UUID2]},
            helper,
            self.top_cols,
            self.json_col,
        )
    ...
    def test_routes_standard_column_shorthand_eq(self):
        helper = ParamHelper()
        sql = _process_field_condition(
            "owner_id", UUID1, helper, self.top_cols, self.json_col
        )
    ...
    def test_routes_metadata_shorthand_eq_implicit(self):
        helper = ParamHelper()
        sql = _process_field_condition(
            "tags", "urgent", helper, self.top_cols, json_column=self.json_col
        )
        expected_sql = f"{self.json_col}->>'tags'=$1"
        assert sql.replace(" ", "") == expected_sql.replace(" ", "")
    ...
    def test_routes_metadata_op_implicit(self):
        ...
        sql = _process_field_condition(
            "score",
            {FilterOperator.GT: 90},
            helper,
            self.top_cols,
            self.json_col,
        )
        accessor = f"{self.json_col}->>'score'"
        expected_sql = self._expected_safe_compare_sql(
            accessor, ">", "$1", "numeric"
        )
        assert sql.replace(" ", "") == expected_sql.replace(" ", "")
    ...
    def test_routes_metadata_nested_shorthand_eq_implicit(self):
        helper = ParamHelper()
        sql = _process_field_condition(
            "nested.value", True, helper, self.top_cols, self.json_col
        )
        accessor = f'{self.json_col}#>>\'{{"nested","value"}}\''
        expected_sql = self._expected_safe_compare_sql(
            accessor, "=", "$1", "boolean"
        )
        assert sql.replace(" ", "") == expected_sql.replace(" ", "")
    ...
    def test_routes_metadata_explicit_path_op(self):
        helper = ParamHelper()
        sql = _process_field_condition(
            f"{self.json_col}.score",
            {FilterOperator.LTE: 100},
            helper,
            self.top_cols,
            json_column=self.json_col,
        )
    ...
    def test_routes_metadata_explicit_column_nested_structure(self):
        ...
        condition_spec = {
            "path.to.key": "val",
            "another": {FilterOperator.NE: False},
        }
        sql = _process_field_condition(
            self.json_col,
            condition_spec,
        ...
        accessor2 = f"{self.json_col}->>'another'"
        expected_part2 = self._expected_safe_compare_sql(
            accessor2, "!=", "$2", "boolean"
        )
...
class TestProcessFilterDict:
    ...
    def test_single_field_condition(self):
        helper = ParamHelper()
        filters = {"id": UUID1}
        sql = _process_filter_dict(
            filters, helper, self.top_cols, self.json_col
        )
        assert sql == "id = $1"
        assert helper.params == [UUID1]
    ...
    def test_multiple_field_conditions_implicit_and(self):
        ...
        assert actual_sql == expected_sql1.replace(
            " ", ""
        ) or actual_sql == expected_sql2.replace(" ", "")
    ...
    def test_nested_logical(self):
        ...
        accessor = f"{self.json_col}->>'score'"
        score_condition = self._expected_safe_compare_sql(
            accessor, ">", "$3", "numeric"
        )
        expected_sql = f"(id = $1) AND ((status = $2) OR ({score_condition}))"
        assert sql.replace(" ", "") == expected_sql.replace(" ", "")
...
class TestApplyFiltersApi:
    ...
    def test_simple_metadata_equality_implicit(self):
        ...
        sql, params = apply_filters(
            filters, [], mode="condition_only", json_column=self.json_column
        )
        expected_sql = f"{self.json_column}->>'key'=$1"
        assert sql.replace(" ", "") == expected_sql.replace(" ", "")
        assert params == ["value"]
    ...
    def test_numeric_metadata_comparison_implicit(self):
        ...
        accessor = f"{self.json_column}->>'score'"
        expected_sql = self._expected_safe_compare_sql(
            accessor, ">", "$1", "numeric"
        )
        assert sql.replace(" ", "") == expected_sql.replace(" ", "")
    ...
    def test_metadata_column_target_nested(self):
        filters = {
            self.json_column: {"path.to.value": {FilterOperator.EQ: 10}}
        }
        sql, params = apply_filters(
            filters, [], mode="condition_only", json_column=self.json_column
        )
    ...
    def test_collection_ids_array_contains(self):
        filters = {
            "collection_ids": {FilterOperator.ARRAY_CONTAINS: [UUID1, UUID2]}
        }
        sql, params = apply_filters(filters, [], mode="condition_only")
        assert sql.replace(" ", "") == "collection_ids@>ARRAY[$1,$2]::uuid[]"
        assert params == [UUID1, UUID2]
    ...
    def test_initial_params_accumulation(self):
        ...
        filters = {"id": UUID1}
        sql, params = apply_filters(
            filters, param_list=initial, mode="condition_only"
        )
        assert sql == "id = $2"
    ...
    def test_custom_top_level_columns(self):
        ...
        sql_m, params_m = apply_filters(
            filters_meta,
            [],
            top_level_columns=custom_columns,
            mode="condition_only",
        )
    ...
    def test_metadata_array_in_implicit(self):
        filters = {"tags": {FilterOperator.IN: ["urgent", "new"]}}
        sql, params = apply_filters(
            filters, [], mode="condition_only", json_column=self.json_column
        )
        expected_sql = f"{self.json_column}->'tags' ?| ARRAY[$1,$2]::text[]"
        assert sql.replace(" ", "") == expected_sql.replace(" ", "")
        assert params == ["urgent", "new"]

    def test_metadata_array_in_explicit_nested(self):
        filters = {
            f"{self.json_column}.version_info.tags": {
                FilterOperator.IN: ["legacy"]
            }
        }
        sql, params = apply_filters(
            filters, [], mode="condition_only", json_column=self.json_column
        )
        expected_sql = f'{self.json_column}#>\'{{"version_info","tags"}}\' ?| ARRAY[$1]::text[]'
        assert sql.replace(" ", "") == expected_sql.replace(" ", "")
        assert params == ["legacy"]

    def test_metadata_array_nin_implicit(self):
        filters = {"tags": {FilterOperator.NIN: ["obsolete"]}}
        sql, params = apply_filters(
            filters, [], mode="condition_only", json_column=self.json_column
        )
        expected_sql = f"NOT ({self.json_column}->'tags' ?| ARRAY[$1]::text[])"
        assert sql.replace(" ", "") == expected_sql.replace(" ", "")
    ...
    def test_metadata_array_nin_explicit_nested(self):
        filters = {
            f"{self.json_column}.options": {
                FilterOperator.NIN: ["disabled", "hidden"]
            }
        }
        sql, params = apply_filters(
            filters, [], mode="condition_only", json_column=self.json_column
        ...
        expected_sql = (
            f"NOT ({self.json_column}->'options' ?| ARRAY[$1,$2]::text[])"
        )
        assert sql.replace(" ", "") == expected_sql.replace(" ", "")
        assert params == ["disabled", "hidden"]

    def test_metadata_array_in_empty(self):
        filters = {"tags": {FilterOperator.IN: []}}
        sql, params = apply_filters(
            filters, [], mode="condition_only", json_column=self.json_column
        )
        assert sql == "FALSE"
    ...
    def test_metadata_array_nin_empty(self):
        filters = {"tags": {FilterOperator.NIN: []}}
        sql, params = apply_filters(
            filters, [], mode="condition_only", json_column=self.json_column
        )
        assert sql == "TRUE"
    ...
    def test_combined_filters(self):
        filters = {
            FilterOperator.AND: [
                {"id": UUID1},
                {f"{self.json_column}.score": {FilterOperator.GTE: 80}},
                {
                    FilterOperator.OR: [
                        {"collection_id": UUID2},
                        {"owner_id": {FilterOperator.EQ: UUID3}},
                    ]
                },
            ]
        }
        ...
        expected_sql = f"(id = $1) AND ({score_condition}) AND ((collection_ids && ARRAY[$3]::uuid[]) OR (owner_id = $4))"
        assert sql.replace(" ", "") == expected_sql.replace(" ", "")
    ...
    def test_combined_filters_with_array_in(self):
        filters = {
            FilterOperator.AND: [
                {"id": UUID1},
                {
                    f"{self.json_column}.labels": {
                        FilterOperator.IN: ["critical"]
                    }
                },
                {
                    FilterOperator.OR: [
        ...
        labels_condition = f"{self.json_column}->'labels' ?| ARRAY[$2]::text[]"
        expected_sql = f"(id = $1) AND ({labels_condition}) AND ((collection_ids && ARRAY[$3]::uuid[]) OR (owner_id = $4))"
        assert sql.replace(" ", "") == expected_sql.replace(" ", "")
        assert params == [UUID1, "critical", UUID2, UUID3]

    def test_more_complex_metadata_and_standard(self):
        filters = {
            "status": {FilterOperator.NE: "archived"},
            "metadata.tags": {FilterOperator.JSON_CONTAINS: ["urgent"]},
            FilterOperator.OR: [
                {f"{self.json_column}.priority": {FilterOperator.GTE: 5}},
                {"owner_id": UUID1},
            ],
        }
        sql, params = apply_filters(
        ...
        tags_condition = f"{self.json_column}->'tags' @> $2::jsonb"
        accessor = f"{self.json_column}->>'priority'"
        priority_condition = self._expected_safe_compare_sql(
            accessor, ">=", "$3", "numeric"
        )
