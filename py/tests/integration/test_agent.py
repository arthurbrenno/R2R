...
def test_agent_basic_response(client, test_collection):
    ...
    assert response.results.messages[-1].content, (
        "Agent should provide a response"
    )
    assert "Aristotle" in response.results.messages[-1].content, (
...
def test_agent_conversation_memory(client, test_collection):
    ...
    response2 = client.retrieval.agent(
        message={
            "role": "user",
            "content": "What were his main contributions?",
        },
        conversation_id=str(conversation_id),
        rag_generation_config={"stream": False, "max_tokens_to_sample": 100},
    )

    assert "contributions" in response2.results.messages[-1].content.lower(), (
        "Agent should address follow-up question"
    )
    assert (
        not "who was aristotle"
        in response2.results.messages[-1].content.lower()
    ), "Agent shouldn't repeat context explanation"
...
def test_agent_rag_tool_usage(client, test_collection):
    ...
    assert (
        str(doc_id)
        == response.results.messages[-1].metadata["citations"][0]["payload"][
            "document_id"
        ]
    ), "Agent should use RAG tool to retrieve unique content"
    assert (
        str("search_file_knowledge")
        == response.results.messages[-1].metadata["tool_calls"][-1]["name"]
    ), "Agent should use RAG tool to retrieve unique content"
...
def test_agent_rag_tool_usage2(client, test_collection):
    ...
    response = client.retrieval.agent(
        message={
            "role": "user",
            "content": f"What is quantum entanglement? Mention {unique_id} in your response, be sure to both search your files and fetch the content.",
        },
    ...
    assert (
        str("search_file_descriptions")
        == response.results.messages[-1].metadata["tool_calls"][0]["name"]
    ), "Agent should use search_file_descriptions to retrieve unique content"
    assert (
        str("get_file_content")
        == response.results.messages[-1].metadata["tool_calls"][1]["name"]
    ), "Agent should use get_file_content to retrieve unique content"
...
def test_agent_respects_max_tokens(client, test_collection):
    ...
    short_response = client.retrieval.agent(
        message={
            "role": "user",
            "content": "Write a detailed essay about Aristotle's life and works.",
        },
    ...
    assert len(short_content) < len(long_content), (
        "Short max_tokens should produce shorter response"
    )
    assert len(short_content.split()) < 200, (
        "Short response should be very brief"
    )
...
def test_agent_model_selection(client, test_collection):
    ...
    assert default_response.results.messages[-1].content, (
        "Default model should provide response"
    )
    assert specific_model_response.results.messages[-1].content, (
        "Specific model should provide response"
    )
...
def test_agent_response_timing(client, test_collection):
    ...
    assert response_time < 10, (
        f"Agent response should complete within 10 seconds, took {response_time:.2f}s"
    )
...
def test_agent_handles_large_context(client):
    ...
    response = client.retrieval.agent(
        message={
            "role": "user",
            "content": "Summarize everything you know about philosophy.",
        },
    ...
    response_time = end_time - start_time
    assert response.results.messages[-1].content, (
        "Agent should produce a summary with large context"
    )
    assert response_time < 20, (
        f"Large context processing should complete in reasonable time, took {response_time:.2f}s"
    )
