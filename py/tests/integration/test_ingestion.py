        ...
        if wait_for_completion:
            ...
            while True:
                try:
                    ...
                    ingestion_status = (
                        retrieval_response.results.ingestion_status
                    )
...
def test_img_ingestion(client: R2RClient):
    ...
    result = client.documents.create(
        ...
        metadata={"title": "Test Document", "author": "Test Author"},
        ingestion_config={
            "vlm": "anthropic/anthropic/claude-3-7-sonnet-20250219"
        },
        run_with_orchestration=False,
    )
...
def test_metadata_title_handling(client: R2RClient):
    ...
    raw_text_doc = next(
        (doc for doc in list_response.results if doc.id == raw_text_doc_id),
        None,
    )
    ...
    list_response = client.documents.list()
    chunks_doc = next(
        (doc for doc in list_response.results if doc.id == chunks_doc_id), None
    )
